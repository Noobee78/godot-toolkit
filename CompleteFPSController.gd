...
@export_range(0.001, 0.3) var mouse_sensitivity := 0.002
@export_range(0.01, 5.0) var joystick_sensitivity := 2.0
...
func _ready():
	_camera.rotation.y = 0.0 # to ensure camera never has yaw
	Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
	_neck_start_height = _neck.position.y
...
func _physics_process(delta: float) -> void:
	...
	#Use body's facing vectors directly
	var forward := -transform.basis.z
	var right := transform.basis.x
	var movement_direction_3d := (right * input_direction_2d.x + forward * -input_direction_2d.y).normalized()
	
	# Calculating max speed and acceleration
	var player_wants_to_move := input_direction_2d.length() > 0.1
...
# Update collision shape's height and position
	if is_crouching:
...
func _rotate_camera_by(look_offset_2d: Vector2) -> void:
	# Yaw on the Characterbody3D (whole body turns)
	rotation.y -= look_offset_2d.x
	rotation.y = wrapf(rotation.y, -PI, PI)
	# Pitch on the camera only
	_camera.rotation.x -= look_offset_2d.y
	
	const MAX_VERTICLE_ANGLE := PI / 3.0
	_camera.rotation.x = clampf(_camera.rotation.x, -1.0 * MAX_VERTICLE_ANGLE, MAX_VERTICLE_ANGLE)
